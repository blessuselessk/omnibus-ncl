...
```
Beginner: nickel-language-handbook-llm.md (this document)
  ↓
Interface Design: nickel-interface-style.md
  ↓
Module Organization: nickel-modules.md
  ↓
...
In Nickel, a `match` block is a first-class function. This is often cleaner than `if-else` for dispatching based on enum tags.

```nickel
let get_config = match {
  'development => { debug = true, port = 8080 },
...
### Row Polymorphism in Functions

Functions can be generic over record "tails" (extra fields).

```nickel
let getName = forall r. { name : String | r } -> String
  = fun obj => obj.name in

getName { name = "Alice", age = 30 } # Works with extra 'age' field
```

### The pipeline operator `|>`

Use `|>` to chain transformations, especially with `std.array` and `std.record`.
