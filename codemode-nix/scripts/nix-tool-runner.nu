...
def main [registry: path, tool_name: string, ...args: string] {
  ...
  if not ($tool_name in ($reg | columns)) {
    ...
    error make {msg: $"unknown tool '($tool_name)'. available: ($available)"}
  }
  ...
  match (sys host | get name) {
    ...
    "Linux" => {
      ...
      if (which nsjail | is-not-empty) {
        mut nsjail_args = [
          --mode ONCE
          --time_limit ($timeout_secs | into string)
          --rlimit_as (($rlimit_as / 1024 / 1024) | into string)
          --rlimit_cpu ($rlimit_cpu | into string)
        ]
        if not $network {
          $nsjail_args = ($nsjail_args | append "--disable_clone_newnet")
        }
    ...
    _ => {
      error make {msg: $"unsupported platform '(sys host | get name)'"}
    }
  }
}
